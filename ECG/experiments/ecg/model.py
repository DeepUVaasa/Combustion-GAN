...
class Discriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.out = nn.Sequential(nn.Conv1d(opt.nz,1,1, bias=False))
    ...
    def forward(self, x):
        ...
        out = self.out(features1)
        classifier = self.sigmoid(out)

        classifier = classifier.view(-1, 1).squeeze(1)

        return classifier, features1 
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        gen_x = self.decoder(latent_ii)
        gen_x = gen_x.view(-1,1,self.opt.isize)

        return gen_x, latent_i.squeeze(2) 
...
class CombustionGAN(AD_MODEL):
    ...
    def update_netd(self):
        ...
        #print(self.out_d_real.size(), self.feat_real.size())
        # --
        # Train with fake
        self.label.data.resize_(self.opt.batchsize).fill_(self.fake_label)
        #self.fake, self.latent_i, kld = self.G(self.input)
    ...
    def update_netg(self):
        ...
        self.err_g =  self.err_g_rec + self.err_g_adv * self.opt.w_adv #+ kld.mean()
        self.err_g.backward()
    ...
    def predict(self,dataloader_,scale=True):
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader_, 0):
                ...
                #self.fake, latent_i,_ = self.G(self.input)
    ...
    #         for i, (data, label,_,_) in enumerate(dataloader_, 0):
    ...
    #             assert batch_output.shape[0]==batch_input.shape[0]==ano_score.shape[0]
    #             for idx in range(batch_input.shape[0]):
    #                 # if len(test_pair_byond_normal)>=100:
    #                 #     break
    #                 normal_score=(ano_score[idx]-min_score)/(max_score-min_score)
    ...
    #                 #test_pair.append((batch_input[idx],batch_output[idx]))
    #             # nfilename = 'normal_test_recon_sample_'+str(i)+ '.npy'
    #             # abnfilename = 'abnormal_test_recon_sample_'+str(i)+ '.npy'
    #             # npath = os.path.join(save_dir, nfilename )
    #             # abnpath = os.path.join(save_dir, abnfilename)
    #             # np.save(npath, test_pair_normal)
    #             # np.save(abnpath,test_pair_byond_normal)
    #             #print(len(test_pair))
    #         print('Number of Abnormal and Normal  Sample detected:',len(test_pair_byond_normal), len(test_pair_normal))
    #         self.saveTestPair(test_pair_byond_normal[:100],save_dir, abnormal_name[:100])
    #         self.saveTestPair(test_pair_normal[:100],save_dir, normal_name[:100])
    ...
    def test_type(self):
        ...
        # y_N_lbl, y_pred_N_lbl=self.predict(self.dataloader["test"],scale=False)
        # aucprc,aucroc,best_th,best_f1=evaluate(y_N_lbl, y_pred_N_lbl)
        #print(y_N_lbl, '\n', y_pred_N_lbl)
        #print(len(y_pred_N), len(y_N), y_pred_N, '\n', y_N)
        # y_S, y_pred_S = self.predict(self.dataloader["test_S"],scale=False)
        # y_V, y_pred_V = self.predict(self.dataloader["test_V"],scale=False)
        # y_F, y_pred_F = self.predict(self.dataloader["test_F"],scale=False)
        # y_Q, y_pred_Q = self.predict(self.dataloader["test_Q"],scale=False)
        over_all= y_pred_N #np.concatenate([y_pred_N,y_pred_S,y_pred_V,y_pred_F,y_pred_Q])
        over_all_gt= y_N #np.concatenate([y_N,y_S,y_V,y_F,y_Q])
        min_score,max_score=np.min(over_all),np.max(over_all)
        ...
        #self.predict_for_right(self.dataloader["test"], min_score,max_score,res_th,save_dir=os.path.join(save_dir, "N"))
        # self.predict_for_right(self.dataloader["test_S"], min_score,max_score,res_th,save_dir=os.path.join(save_dir, "S"))
        # self.predict_for_right(self.dataloader["test_V"], min_score,max_score,res_th,save_dir=os.path.join(save_dir, "V"))
        # self.predict_for_right(self.dataloader["test_F"], min_score,max_score,res_th,save_dir=os.path.join(save_dir, "F"))
        # self.predict_for_right(self.dataloader["test_Q"], min_score,max_score,res_th,save_dir=os.path.join(save_dir, "Q"))
    ...
    #     #evaluate(over_all_gt,(over_all-min_score)/(max_score-min_score), saveto=save_dir)
    """
    def test_time(self):
        self.G.eval()
        self.D.eval()
