...
def save_plot_sample(samples, idx, identifier, n_samples=6, num_epochs=None,impath=None ,ncol=2):
    ...
    fig, axarr = plt.subplots(nrow, ncol, sharex=True, figsize=(16, 16)) #
    if identifier=="iscp":
        for m in range(nrow):
            for n in range(ncol):
...
def save_plot_pair_sample(samples1,samples2, idx, identifier, n_samples=6, num_epochs=None,impath=None ,ncol=2):
    ...
    nrow = int(n_samples/ncol)
    fig, axarr = plt.subplots(nrow, ncol, sharex=True, figsize=(32, 32)) #
    if identifier=="iscp":
        for m in range(nrow):
...
def save_ts_heatmap(input,output,save_path):
    ...
    fig, ax = plt.subplots(2, 1, sharex=True,gridspec_kw = {'height_ratios':[6,1]}, figsize=(8, 6)) #figsize=(6, 6),
    #fig, ax = plt.subplots(1, 1, figsize=(8, 6)) #figsize=(6, 6),
    sig_in = input[0, :]
    sig_out = output[0, :]
    ax[0].plot(x_points, sig_in,'g-',linewidth=1.5,label="Input P")
    ax[0].plot(x_points,sig_out,'k--',linewidth=1.5,label="Reconstructed P'")
    #ax[0].set_yticks([])
    ...
    ax[1].set_yticks([])
    ax[1].xaxis.set_major_formatter(my_formatter)
    fig.tight_layout()
