...
class Encoder(nn.Module):
    def __init__(self, ngpu,opt,out_z):
        ...
        self.main = nn.Sequential(
            nn.Linear(opt.isize, chans[0], bias=False),
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[0]]),

            nn.Linear(chans[0],chans[1], bias=False),
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[1]]),

            nn.Linear(chans[1],chans[2], bias=False),
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[2]]),

            nn.Linear(chans[2],chans[3], bias=False),
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[3]]),

            nn.Linear(chans[3],chans[4], bias=False), 
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[4]]),

            nn.Linear(chans[4],chans[5], bias=False),
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[5]]),

            nn.Linear(chans[5],chans[6], bias=False), 
            nn.LeakyReLU(0.2),
            nn.LayerNorm([chans[6]]),

            nn.Linear(chans[6],self.out_z, bias=False)
...
class Decoder(nn.Module):
    def __init__(self, ngpu,opt):
        ...
        self.tanh = nn.Tanh()
        self.dl1 = nn.Linear(opt.nz, chans[6], bias=False)

        self.dl2  = nn.Linear(chans[6], chans[5], bias=False)

        self.dl3 = nn.Linear(chans[5]+chans[6], chans[4], bias=False)

        self.dl4 = nn.Linear(chans[4]+chans[5], chans[3], bias=False)


        self.dl5 = nn.Linear(chans[3]+opt.nz, chans[2], bias=False)


        self.dl6 = nn.Linear(chans[2]+chans[3], chans[1], bias=False)


        self.dl7 = nn.Linear(chans[1]+chans[2], chans[0], bias=False)

        self.dl8 = nn.Linear(chans[0]+chans[1],opt.isize, bias=False)
...
class Combustion_MODEL(object):
    ...
    def visualize_pair_results(self,epoch,samples1,samples2,is_train=True):
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        #print(os.path.join(save_dir,'epoch%03d' % epoch + '.png'))
